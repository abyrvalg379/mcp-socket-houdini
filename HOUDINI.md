...
6. `hou.FlipbookSettings` абстрактный — конструктора нет. НО у
   ...
   ⚠ НА ЭТОЙ МАШИНЕ (20.5.278, 2026-09-24) ФАЙЛОВЫЙ выход флипбука
   молча Failed на записи (даже с заведомым путём без переменных,
   MPlay off, range 1-1, папка создана; mplay-выход при этом работает;
   исключений нет, crash-логов нет) — тул flipbook в файл на этой
   среде рассчитывать нельзя, использовать mode='window'. Раскопать
   причину (плагин/драйвер/конфиг) — при случае, не блокер.
